...
- **Known non-determinism (not yet fixed):** `queries.sql` Query 2's delay-bucket assignment (same pattern also present in Query 4) uses `ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY order_date)` with no tie-breaker for same-day orders, so exact bucket percentages can drift slightly between runs. Verified empirically: the dashboard and LinkedIn carousel report 39.82% / 33.86% / 28.89% / 25.53% / 20.09% for the five delay buckets; a fresh run of the same committed query currently returns 39.92% / 33.89% / 28.95% / 25.67% / 20.55% — a spread of up to ~0.5 percentage points, largest in the 50+ minute bucket. Adding an explicit tie-breaker (`ORDER BY order_date, order_id`) narrows but does not eliminate this. The conclusion itself — delay is a real, gradual, non-catastrophic churn driver — is unaffected. Left unfixed to keep this hardening pass scoped to its stated phases.
